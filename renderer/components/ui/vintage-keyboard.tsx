...
/** Key id -> KeyboardEvent.code, so callers can map back to their own codes. */
export const KEY_ID_TO_CODE: Record<string, string> = (() => {
  const map: Record<string, string> = {};
  for (const [code, id] of Object.entries(CODE_TO_KEY_ID)) {
    if (!(id in map)) map[id] = code;
  }
  return map;
})();
...
function useDeviceTier(): DeviceTier {
  // Must match the server's value for the first render; the effect below
...
const Key = memo(function Key({
  ...
  onPointerPress: (id: string, muted: boolean, source: "pointer") => void;
  onPointerRelease: (id: string, source: "pointer") => void;
}) {
  const {
    id,
    label,
  ...
  const handlePress = useCallback(
    (event: ReactPointerEvent<HTMLButtonElement>) => {
      ...
      pressPointer();
      onActivate(id);
      onPointerPress(id, !!muted, "pointer");
    },
    [id, muted, pressPointer, onActivate, onPointerPress],
  );

  const handleRelease = useCallback(() => {
    releasePointer();
    onDeactivate(id);
    onPointerRelease(id, "pointer");
  }, [releasePointer, onDeactivate, onPointerRelease, id]);
...
export interface VintageKeyboardProps {
  /**
   * Suppress the bundled sample so a host app can drive its own audio from
   * `onKeyPress`. Without this the board plays its own single thock sample.
   */
  silent?: boolean;
  /** Show the "Press any key..." chip above the board. */
  showIndicator?: boolean;
  /**
   * Wrap the board in a full-viewport, light-background frame. Turn this off to
   * drop the board into an existing page section.
   */
  standalone?: boolean;
  /**
   * Fires on every press. `source` distinguishes a clicked cap from a real
   * keystroke, so a host driving its own audio can avoid doubling up with a
   * listener of its own.
   */
  onKeyPress?: (keyId: string, source: "pointer" | "keyboard") => void;
  /** Fires when a pressed key is released, with the same `source` tagging. */
  onKeyRelease?: (keyId: string, source: "pointer" | "keyboard") => void;
  className?: string;
}
...
export const VintageKeyboard = ({
  ...
  // Kept in refs so the global key listener never needs to re-subscribe.
  ...
  const emitPress = useCallback(
    (id: string, muted: boolean, source: "pointer" | "keyboard") => {
      if (!silentRef.current) {
        playKeySound(getSoundCategory(id), muted, KEY_PAN[id] ?? 0);
      }
      onKeyPressRef.current?.(id, source);
    },
    [],
  );

  const emitRelease = useCallback(
    (id: string, source: "pointer" | "keyboard") => {
      onKeyReleaseRef.current?.(id, source);
    },
    [],
  );
  ...
  useEffect(() => {
    ...
    const handleKeyDown = (event: KeyboardEvent) => {
      ...
      emitPress(id, !!config?.muted, "keyboard");
    };
  ...
  }, [activateKey, deactivateKey, emitPress, emitRelease]);

  const board = (
    <div
      className="flex flex-col items-center"
      style={{ width: "100%", maxWidth: standalone ? container.maxWidth : "100%" }}
    >
      {showIndicator && (
        <div
...
`,
          ...
          >
            ...
            />
            <div className="pointer-events-none absolute inset-[1px] rounded-[calc(var(--kb-case-radius)_-_0.06rem)] border-t border-l border-[#f6dfae]/20" />
            <div className="pointer-events-none absolute inset-[1px] rounded-[calc(var(--kb-case-radius)_-_0.06rem)] border-b border-r border-[#3f2811]/30" />
            <div
                  ...
                  <div key={i} className="flex" style={{ gap }}>
                    {row.map((key) => (
                      <Key
                        ...
                        onPointerPress={emitPress}
                        onPointerRelease={emitRelease}
                      />
                    ))}
                  </div>
                ))}
  ...
  if (!standalone) {
    return (
      <div className={`flex w-full justify-center ${className}`}>
        <style dangerouslySetInnerHTML={{ __html: KEY_STYLE_TAG }} />
        {board}
      </div>
    );
  }
  ...
  return (
    <div
      className={`kb-viewport flex w-full items-center justify-center overflow-x-hidden bg-[#FAFAFA] ${className}`}
      style={{
...
/** Original demo export — full-viewport board with its own bundled sound. */
